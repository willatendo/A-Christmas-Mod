...
public class ChimneyBlock extends Block implements IWaterLoggable
{
	...
	public enum ChimneyBlocks implements IBlockRegister
	{
		BRICK_CHIMNEY(new ChimneyBlock(AbstractBlock.Properties.of(Material.STONE, MaterialColor.COLOR_RED).requiresCorrectToolForDrops().strength(2.0F, 6.0F).noOcclusion())),
		STONE_BRICK_CHIMNEY(new ChimneyBlock(AbstractBlock.Properties.of(Material.STONE).requiresCorrectToolForDrops().strength(1.5F, 6.0F)));
		
		private final Block block;
		
		private ChimneyBlocks(Block block) 
		{
			this.block = block;
		}
